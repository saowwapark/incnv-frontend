...
export class PageDataSource<T> extends DataSource<T> {
  ...
  constructor(
    private _service: PageDataSourceService<T>,
  ...
  private sortData(data: any[]): any[] {
    ...
    console.log('MatSort.active: ' + this._matSort.active);
    console.log('MatSort.direction: ' + this._matSort.direction);
    console.log('MatSort');
    ...
    return copiedData.sort((a, b) => {
      ...
      [propertyA, propertyB] = [
        a[this._matSort.active],
        b[this._matSort.active]
      ];
      if (typeof propertyB === 'number' && typeof propertyA === 'number') {
        return (
          (propertyB - propertyA) * (this._matSort.direction === 'asc' ? 1 : -1)
        );
      } else {
        return (
          ...
          (propertyB as string).toLowerCase()
            ? -1
            : 1) * (this._matSort.direction === 'asc' ? 1 : -1)
        );
      }
